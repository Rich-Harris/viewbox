...
( function( global ) {
	...
	var set = function set( viewBox, box ) {
		var i, len;
		if ( viewBox.x === box.x && viewBox.y === box.y && viewBox.width === box.width && viewBox.height === box.height ) {
			return;
		}
		viewBox.x = box.x;
		viewBox.y = box.y;
	...
	var ViewBox$observe = function ViewBox$observe( callback, options ) {
	...
	var fire = function fire( viewBox, eventName ) {
		var callbacks = viewBox._callbacks[ eventName ],
			args;
		if ( !callbacks ) {
			return;
		}
	...
	var interaction_mouseDrag = function( clean, fire ) {
		...
		return function draggable( viewBox ) {
			...
			function mousemoveHandler( event ) {
				...
				// prevents user from selecting other parts of the document
	...
	var ViewBox = function( animate, getClientCoords, getSvgCoords, getZoom, observe, pan, set, toJSON, toString, zoom, mouseDrag, touch, applyInertia, clean, easing, getViewBoxFromSvg ) {
		...
		return ViewBox;
	}( ViewBox$animate, ViewBox$getClientCoords, ViewBox$getSvgCoords, ViewBox$getZoom, ViewBox$observe, ViewBox$pan, ViewBox$set, ViewBox$toJSON, ViewBox$toString, ViewBox$zoom, interaction_mouseDrag, interaction_touch, interaction_applyInertia, clean, easing, getViewBoxFromSvg );
